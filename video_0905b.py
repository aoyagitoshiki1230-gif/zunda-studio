...
def merge_number_phrases(q):
    ...
    out = []
    for ap in aps:
        if out and _is_number_only(_phrase_kana(out[-1])) and not out[-1].get("pause_mora"):
            prev = out.pop()
            k = len(prev["moras"])
            ap = dict(ap)
...
def main(script_path, out_path, workdir=None):
    ...
    for i, (sc, items, scene_dur) in enumerate(plan, 1):
        ...
        for j, (who, e2, sent, wav, q, dur) in enumerate(items):
            ...
            # 文の中身に応じた小さな差し込み。直前の音と近すぎるときは鳴らさない
            ...
            if j > 0 and who != prev_who[0]:
                _add(at + 0.02, "blip", 1.4)          # 話し手が入れ替わった
            if re.search(r"[0-9]", sent):             # 数字が出てくる文
                _add(at + dur * 0.55, "coin" if "円" in sent else "tick", 1.0)
            if sent.rstrip().endswith(("!", "！")):    # 言い切ったところ
                _add(at + dur * 0.86, "wood", 1.0)
            prev_who[0] = who
            if j == 0 and i in card_at:
