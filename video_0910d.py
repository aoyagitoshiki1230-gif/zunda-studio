...
# 41秒(視聴9.7秒・再生1392・登録1人)と39秒(視聴10.3秒・再生1519)。長くしても再生は落ちない
# (1日の本数を8本に増やした日も1本平均1025回で、本数と1本平均の相関は-0.15しかない)
SHORT_MAX    = 52.0       # ここを超えたら打ち切る
SHORT_TARGET = 42.0       # 狙う尺
...
def build_short(script_path, out_path, workdir=None):
    ...
    if len(items) < len(sents):
        print(f"  ナレーションが長いので{len(items)}/{len(sents)}文で打ち切り "
              ...
              f"{int(SHORT_TARGET)}秒(205字前後)に収めるとよい")
