...
# 2026-09-10の実測: この語が入った長編6本は平均視聴55秒、入っていない22本は88秒だった。
# 「〜を解説」「〜入門」「〜の正体」は中身が同じでも見られない
WEAK_WORDS = ("解説", "入門", "まとめ", "正体", "手口", "徹底", "の実態", "仕組み")
...
def check_titles(titles):
    ...
    for t in titles:
        ...
        weak = [w for w in WEAK_WORDS if w in t]
        if weak:
            note += f" / 説明っぽい語: {'・'.join(weak)}"
        # 2026-09-15の実測: タイトルに「#ずんだもん」等のハッシュタグが混ざった3本のうち
        # 9月11日公開の1本は350回しか回らなかった(同日の他の5本は971〜1144回)。
        # ハッシュタグは説明欄に書くもので、タイトルに入れてはいけない
        if "#" in t or "＃" in t:
            note += " / ★タイトルにハッシュタグが入っている。説明欄へ移すこと"
        # 同じく2026-09-15の実測: 同じ日に出した本どうしの比較で
        # 数字入り=平均968回 ↔ 数字なし=656回。数字は必ず入れる
        if not re.search(r"[0-9０-９]", t):
            note += " / 数字が入っていない(同日比較で再生が3割下がる)"
        out.append((t, n, note))
    return out
...
def build_description(meta, chapters):
    ...
    terms = [t for t in (meta.get("terms") or [])
    ...
    L += ["", "━━━━━━━━━━━━━━━━", "◆ ご注意", "━━━━━━━━━━━━━━━━",
          (_lines(meta.get("notice")) or DEFAULT_NOTICE).rstrip()]
    credits = list(meta.get("credits") or [])
    if not any("VOICEVOX" in c for c in credits):
        credits.insert(0, REQUIRED_CREDIT)
    L += ["", "━━━━━━━━━━━━━━━━", "◆ クレジット", "━━━━━━━━━━━━━━━━"] + credits
    # 台本が「#税」のように#付きで書いてくることがある。二重に付けない
    tags = [str(t).lstrip("#＃").strip() for t in (meta.get("hashtags") or [])[:TAG_MAX]]
...
def build(meta, chapters, outdir):
    ...
    checked = check_titles(meta.get("titles") or [])
    ...
    warn = []
    if len(meta.get("hashtags") or []) > TAG_MAX:
...
def short_titles(meta, short):
    ...
    short  = short or {}
    long_t = [_norm(t) for t in (meta.get("titles") or [])]
    cands  = [t for t in (short.get("titles") or []) if isinstance(t, str) and t.strip()]
    if not cands:
        base = to_digits(short.get("title") or "") or (meta.get("hook_big") or [""])[0]
        base = base.strip()
        if base:
            cands = [base, base + "【1分で】"]
    out = []
...
def build_short_meta(meta, short, long_title, outdir):
    ...
    sdesc = (f"くわしくは長編で解説しています。\n"
             f"▶ {long_title}\n"
             f"（チャンネルの動画一覧から見られます）\n\n"
             f"#Shorts" + "".join(" #" + str(t).lstrip("#＃").strip()
                                  for t in (meta.get("hashtags") or [])[:3]
                                  if str(t).strip()))
    L += ["", "## 説明欄(このまま貼り付け)", "", "```", sdesc, "```", ""]
    L += ["## ショートの固定コメント", "", "```",
          "この話の続き（なぜそうなるのか）は長編にまとめました。\n"
          "気になった人はチャンネルの動画一覧から見てほしいのだ。", "```", ""]
