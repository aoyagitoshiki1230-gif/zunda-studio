...
# ============ 挿絵(いらすとや素材) ============
# リポジトリに置かれているいらすとや素材は **180x180 しかない**(サイトの一覧用サムネ)。
# 平面的な絵なので 2.5倍(450px)までなら引き伸ばしても持つが、3倍を超えると輪郭が濁る。
# 実測(2026-09-22): 300px=きれい / 450px=実用 / 600px=輪郭が甘い / 800px=明らかに劣化。
# だから **描く大きさは 450px で打ち切る**。原寸の素材が用意できたらこの上限を上げてよい。
ART_MAX   = 450
_ART_INDEX = None


def art_index():
    """素材のファイル名 -> パス の対応表。いらすとやのフォルダはリポジトリ直下にある
    (video.py の ROOT は studio/ なので、その親も探す)"""
    global _ART_INDEX
    ...
    for r in roots:
        for ext in ("png", "jpg", "jpeg"):
            for q in glob.glob(os.path.join(r, "**", f"*.{ext}"), recursive=True):
                base = os.path.basename(q)
                stem = os.path.splitext(base)[0]
                # 「 - コピー」「 - コピー (2)」が付いた重複は同じ絵なので束ねる
                stem = re.sub(r"(\s*-\s*コピー(\s*\(\d+\))?)+$", "", stem).strip()
                # ストックサイトのプレビュー(240_F_<番号>_<英数字>.jpg)は素材ではないので入れない。
                # ライセンスされていないうえ240pxしかなく、台本から名前で引かれると事故になる
                if re.match(r"^\d+_F_\d+_[A-Za-z0-9]{20,}$", stem):
                    continue
                if not stem or stem in idx:
                    continue
                idx[stem] = q
    _ART_INDEX = idx
    return idx
...
# いらすとやのファイル名によく出るローマ字 → 日本語の手がかり。
# 台本を書くときに「年金の挿絵が欲しい」から名前を引けるようにするための表
ART_WORDS = {
    # お金・制度
    "年金": ("nenkin",), "年金手帳": ("nenkin_techou",),
    "世代間": ("kataguruma", "kibasen", "mikoshi"),   # 現役世代が支える比喩
    "税": ("zei", "kokuzei", "genzei"), "確定申告": ("kakuteishinkoku", "etax"),
    "控除": ("kouzyo", "koujo"), "保険": ("hoken",), "保険証": ("hokensyou",),
    "銀行": ("ginkou", "atm", "tsuchou", "chokin"), "株": ("kabu",), "投資": ("toushi", "ideco", "nisa"),
    "お金": ("money", "satsu", "coin", "saifu", "yen"),
    "家計": ("kakei", "kakeibo"), "値上げ": ("neage", "seikyusyo"),
    "買い物": ("shopping", "kaimono"),
    # 医療・介護・高齢者
    "医療": ("medical", "iryou", "doctor", "nurse", "hospital", "byouin"),
    "病院": ("byouin", "medical", "nyuuin", "hospital", "doctor"),
    "入院": ("nyuuin",), "薬": ("kusuri", "chusyaki"),
    "介護": ("kaigo", "care_manager", "kaigoshi", "fukushishi", "kurumaisu"),
    "車椅子": ("kurumaisu",), "施設": ("shisetsu", "tatemono_kaigo"),
    "通院": ("tsuuin",), "訪問診療": ("houmon_iryou",),
    "認知症": ("haikai",), "デイサービス": ("day_service",),
    "高齢者": ("obaasan", "ojiisan", "kourei", "old", "senior"),
    "免許返納": ("menkyo_hennou", "mark_kourei", "mark_momiji"),
    # 相続・終活
    "相続": ("isan", "souzoku", "seinenkoukennin"), "遺言": ("yuigon",),
    "お墓": ("ohaka", "noukotsu"), "葬儀": ("osoushiki", "otsuya", "butsudan", "reikyusya"),
    # 詐欺・トラブル
    "詐欺": ("sagi", "akutoku", "ayashii", "damasu", "phishing"),
    "泥棒": ("dorobou", "hanzai"), "電話": ("denwa",), "訪問": ("houmon",),
    # 手続き・書類・期限(2026-09-23に素材が大幅に増えたぶん)
    "役所": ("shiyakusho", "yakusho", "madoguchi", "building_gyousei", "uketsuke"),
    "窓口": ("madoguchi", "uketsuke", "counter", "soudan_setsumei"),
    "相談": ("soudan", "counselor"),
    "書類": ("document", "shorui", "syorui", "shinseisyo", "noufusyo", "syoumei", "todoke"),
    "申請書": ("shinseisyo", "syorui_morau", "syorui_yomu"),
    "源泉徴収票": ("gensen_chousyuhyou",), "課税証明": ("kazei_syoumeisyo", "hikazei_syoumeisyo"),
    "納付書": ("noufusyo",), "確定申告書": ("zei_shinkoku", "kakuteishinkoku"),
    "戸籍": ("koseki", "kosekitouhon", "kosekisyouhon"), "登記": ("touki",),
    "封筒": ("fuutou", "futou", "yuubin", "yubin", "envelop"),
    "はんこ": ("hanko", "inkan", "natsuin"),
    "カレンダー": ("calender", "calendar", "nengatsuhi", "week_"),
    # 「期限」で thumbnail_kigen(使用期限・賞味期限の文字絵)は拾わない。日付の期限とは別物
    "期限": ("calender_aseru", "calender_shock", "calender_kabekake"),
    "スマホ": ("smartphone", "mynumber", "pasokon"),
    # 表情・しぐさ
    "困る": ("komaru", "atama_kakaeru", "shinpai", "fuan"),
    "驚く": ("bikkuri", "odoroku", "gakuzen"), "喜ぶ": ("happy", "yorokobu", "egao", "smile"),
    "怒る": ("angry", "okoru", "ikari"), "考える": ("kangae", "nayami", "hatena"),
    "若者": ("gakusei", "student", "boy", "girl", "young"),
    # ライフライン・トラブル(2026-09-23に素材が増えたぶん)
    "水道": ("suidou", "jaguchi", "mizu_"), "電気": ("denki", "denkyu"), "ガス": ("gas",),
    "ライフライン": ("lifeline", "suidou", "denki", "gas"),
    "督促": ("tokusokujou", "toritate"), "通販": ("tsuuhan", "shopping_tsuuhan"),
    "大家": ("ooya", "chintai", "fudousan"),
    # 図に使えるもの
    "グラフ": ("chart", "graph"), "家": ("house", "ie_", "manshion"),
    "車": ("car_", "kuruma"), "会社": ("kaisya", "business", "office"),
}


def art_search(*words, limit=40):
    """素材名を探す。日本語でもローマ字でも引ける。
    台本を書く前に候補を見るために使う:
        python3 -c "import sys;sys.path.insert(0,'lib');import video;print(video.art_search('年金'))"
    """
    idx = art_index()
    keys = []
    for w in words:
        w = str(w).strip()
        keys += list(ART_WORDS.get(w, ())) or [w.lower()]
    hit = [n for n in sorted(idx) if any(k in n.lower() for k in keys)]
    return hit[:limit]


def load_art(name, height):
    """挿絵を1枚、指定の高さで返す(見つからなければ None)。
    180pxの素材を引き伸ばすので、輪郭を立て直してから返す"""
    if not name:
        return None
    from PIL import Image, ImageFilter
    q = art_index().get(str(name).strip())
    if not q:
        print(f"  ⚠ 挿絵が見つからないので図解なしで続けます: {name}", flush=True)
        return None
    try:
        im = Image.open(q).convert("RGBA")
    except Exception as e:
        print(f"  ⚠ 挿絵を開けませんでした({name}): {type(e).__name__}", flush=True)
        return None
    # ★ 透明な余白を先に切る。切らないと「年金手帳」のような縦長の絵が
    # 450pxの枠の中で160pxくらいにしか見えない(2026-09-23に実測して修正)
    bb = im.split()[3].getbbox()
    if bb:
        im = im.crop(bb)
    h = int(min(height, ART_MAX))
    if height > ART_MAX:
        print(f"  挿絵は{ART_MAX}pxで打ち切りました(素材が180pxしかないため): {name}",
              flush=True)
    # 横長の絵は高さで合わせると幅があふれる。枠の幅(図解と同じ1116px)も超えないようにする
    w = max(1, round(im.width * h / im.height))
    if w > 1116:
        h = max(1, round(h * 1116 / w)); w = 1116
    o = im.resize((w, h), Image.LANCZOS)
    rgb = o.convert("RGB").filter(ImageFilter.UnsharpMask(radius=3, percent=110,
                                                          threshold=2))
    out = rgb.convert("RGBA")
    out.putalpha(o.split()[3])
    return out
...
def draw_slide(scene, idx, total, chapter, path, shown=None, progress=0.0,
               speaker=None, badge=None, telop_base_img=None):
    """shown: 表示する箇条書きの数(Noneなら全部) / progress: 全体の進み具合 0〜1
    telop_base_img: テロップの下敷きの画像(telop_base()の戻り)。渡すと最後に焼き込む"""
    ...
    # 挿絵(scene["art"])は図解の代わり。**両方は置かない**(場所が1つしかない)
    art  = None if fig else (scene.get("art") or None)
    ...
    art_im = None
    if art:
        # 挿絵の高さは、箇条書きを置いたあとの余白か450pxの小さいほう
        avail = max(120, LIMIT - TOP - ((len(bullets) * 66 + 24) if bullets else 24))
        if avail < 260:
            # 箇条書きと2行の見出しで場所が残っていない。小さすぎて何の絵か分からなくなる
            print(f"  ⚠ 挿絵の場所が{avail}pxしかありません({art})。"
                  f"箇条書きを0〜1個に減らすか、見出しを1行に収めてください", flush=True)
        art_im = load_art(art, min(avail, ART_MAX))
    ...
    if bullets:
        ...
        if not fig and art_im is None:     # 図解も挿絵も無いときは縦方向に中央寄せ
    ...
    # 挿絵(図解と同じ枠の中で中央に置く。箇条書きが出そろってから)
    if art_im is not None and n >= len(bullets):
        fig_y = min(max(fig_y, TOP + 12), max(TOP + 12, LIMIT - fh))
        ax = 268 + max(0, (FIGW - art_im.width) // 2)
        # 背景は RGB なので alpha_composite は使えない。マスク付きの paste にする
        img.paste(art_im, (ax, fig_y), art_im)
    ...
    # テロップの下敷き。いちばん最後に焼き込む(名札や図解の上に来る)。
    # 位置は ASS の \an2 の中心に合わせる: 横は _TELOP_CX、縦は _TELOP_CY
    if telop_base_img is not None:
        bw, bh = telop_base_img.size
        img.paste(telop_base_img,
                  (int(_TELOP_CX) - bw // 2, int(_TELOP_CY) - bh // 2),
                  telop_base_img)
...
READINGS = [
    ...
    # 消費者ホットラインの188は既定で「ヒャクハチジュウハチ」と読まれる(2026-09-17に報告)。
    # 正しくは1桁ずつの「イチハチハチ」。ただし裸の188を置き換えると「188万円」まで壊れるので、
    # **「188番」の複合語だけ**を登録してある。台本では必ず「188番」と書くこと。
    # 110番(ヒャクトオバン)・119番(ヒャクジュウキュウバン)は既定で正しいので触らない
    ("188番",   "イチハチハチバン",      0),
    # 「1組」は既定で「イックミ」(2026-09-18に報告)。税法の「一個又は一組」は「ヒトクミ」なのだ。
    # 「2人1組」→フタリヒトクミ も直る。**「1年1組」のような組の名前はイチクミが正しいが、
    # 既定でもイックミで間違っていたので、そこは元から直っていない**(学級名は台本にほぼ出ない)
    ("1組",     "ヒトクミ",             2),
    ...
    ("eTax",    "イータックス",         0),
    # 「〜品」は、辞書に無い複合語だと2語に切れて「品」がシナになる(2026-09-22に総点検)。
    # 正しく読める語が大半なので、崩れた複合語だけを足す。
    # 確認ずみで正しいもの: 商品/食品/食料品/日用品/医薬品/化粧品/返品/部品/新品/全品/1品〜20品/
    # 品目/品質/品番/貴重品/遺品/骨董品/不用品/市販品/先発品/後発品/正規品/ブランド品ほかカタカナ＋品
    # 「〜月」が訓読みの「ツキ」に倒れる型(2026-09-23に報告を受けて総点検)。
    # **「偶数月」は年金の振込月なので必ず出る**。「誕生月」は辞書どおり「タンジョウツキ」で正しいので触らない。
    # 正しく読める語: 毎月/今月/来月/先月/前月/翌月/月々/月額/月収/月割り/月曜/満月/
    # 1か月/3か月/数か月/半月/10月/今月末/生まれ月/月20日/その月
    ("偶数月", "グウスウゲツ",         5),   # 既定は「グウスウツキ」。年金は偶数月に振り込まれる
    ("奇数月", "キスウゲツ",           4),   # 既定は「キスウツキ」
    ("支給月", "シキュウゲツ",         4),   # 既定は「シキュウツキ」
    ("月末",   "ゲツマツ",             0),   # 既定は「ツキマツ」。「今月末」は元から正しい
    ("各月",   "カクゲツ",             1),   # 既定は「カクツキ」
    ("生鮮品",  "セイセンヒン",         0),   # 既定は「セイセンシナ」
    ("高級品",  "コウキュウヒン",       0),   # 既定は「コオキュウ・シナ」
    ("純正品",  "ジュンセイヒン",       0),   # 既定は「ジュンセイ・シナ」
    ("非純正品", "ヒジュンセイヒン",     0),   # 既定は「ヒジュンセイ・シナ」
    ("不要品",  "フヨウヒン",           0),   # 既定は「フヨオ・シナ」(「不用品」は元から正しい)
    ...
    ("順張り",     "ジュンバリ",       0),
    # 法令・団体の正式名称
    ("雇用安定",   "コヨウアンテイ",   0),   # 「安定」がヤスジョウになる(高年齢者雇用安定法)
    ("借地借家法", "シャクチシャッカホウ", 0),  # 既定は「シャクヤホウ」
    ("借家人",     "シャクヤニン",     0),   # 〜人がジンになる型
    ("日本年金機構", "ニホンネンキンキコウ", 0),  # 既定は「ニッポン」(正式はニホン)
    ("日本学生支援機構", "ニホンガクセイシエンキコウ", 0),
    ("日本証券業協会", "ニホンショウケンギョウキョウカイ", 0),
    ("売電",       "バイデン",         0),   # 既定は「ウレデン」(太陽光の売電)
    ("卒FIT",     "ソツフィット",     0),   # 既定は「ソツエフアイティー」

    # ---- 「大」が音読みの「ダイ」に倒れる型(2026-09-23に約90語を総点検) ----
    # 大半/大手/大家/大金/大幅/大台/大損/大口/大枠/大方/大元/大通り/大株?/大急ぎ/大慌て/
    # 大騒ぎ/大喧嘩/大物/大昔/大盛り/大詰め/大晦日/大掃除/大盤振る舞い/大安売り は元から正しい
    ("大赤字",     "オオアカジ",       0),   # 既定は「ダイアカジ」
    ("大黒字",     "オオクロジ",       0),   # 既定は「ダイクロジ」
    ("大違い",     "オオチガイ",       3),   # 既定は「ダイチガイ」
    ("大助かり",   "オオタスカリ",     0),   # 既定は「オオダスカリ」(連濁しない)
    ("大荷物",     "オオニモツ",       0),   # 既定は「ダイニモツ」
    ("大津波",     "オオツナミ",       0),   # 既定は「ダイツナミ」
    ("大株主",     "オオカブヌシ",     0),   # 既定は「ダイカブヌシ」

    # ---- 「分」が時間の「フン」に倒れる型 ----
    # 半分/自分/十分/部分/成分/水分/塩分/気分/身分/多分/随分/幾分/配分/区分/処分/分割/
    # 分譲/按分/持分/取り分/差分/九分九厘 と「〜分(ブン)」の複合語(不足分・控除分・前年分ほか)は元から正しい
    ("5分5分",     "ゴブゴブ",         0),   # 既定は「ゴフンゴフン」
    ("腹八分",     "ハラハチブ",       0),   # 既定は「ハラハップン」

    # ---- 「人」が「ジン」に倒れる型のつづき(型1。既存の名義人・相続人ほかに追加) ----
    ("申請人",     "シンセイニン",     0),   # 既定は「シンセイジン」(相続登記の正式用語)
    ("依頼人",     "イライニン",       0),   # 既定は「イライジン」
    ("執行人",     "シッコウニン",     0),   # 既定は「シッコウジン」
    ("遺言執行人", "イゴンシッコウニン", 0),  # 既定は「ユイゴンシッコウジン」
    ("遺言執行者", "イゴンシッコウシャ", 0),  # 「遺言」単独はイゴンと読むので合わせる
    ("他人事",     "ヒトゴト",         0),   # 既定は「タニンゴト」(本則はヒトゴト)
    ("二人暮らし", "フタリグラシ",     0),   # 既定は「フタリ・クラシ」と2語に切れる

    # ---- 「放って」が「ハナッテ」になる(2026-09-23に報告。型3の実害例) ----
    # 「放っておく」は文の前後で読みが変わる(「放っておくと時効になる」は正しいのに
    # 「出さずに放っておくと損をする」はハナッテになった)。
    # **「放って」だけの登録は効かない**(形態素が「放っ+て+おく」に切れるため黙って無視される)。
    # **複合語を活用形ごとに全部登録するしかない。** しかも短い例文では正しく読めるのに
    # 実際の台本の長い文ではハナッテになる(2026-09-23に実測)ので、
    # **確認は必ず台本の文そのままで行うこと**。短い例文で「直った」と判断すると見落とす
    ("放っておく",   "ホウッテオク",   0),
    ("放っておいた", "ホウッテオイタ", 0),
    ("放っておいて", "ホウッテオイテ", 0),
    ("放っておけば", "ホウッテオケバ", 0),
...
; テロップは2枚重ね。下(TelopEdge)が外側の暗いフチと影、上(Telop)が白フチと文字。
; ASSのコメントは「;」。「#」を書くとこのセクションの解析が壊れてStyleが消える(2026-09-23に踏んだ)
Style: TelopEdge,Noto Sans CJK JP,80,&H00152410,&H000000FF,&H00152410,&H64000000,-1,0,0,0,100,100,1,0,1,13,4,2,90,640,84,1
...
def esc(t):
    """ASSのDialogue行に入れられる形にする。
    生の改行やタブが混じると字幕ファイルの行が壊れて、その先が全部出なくなる"""
    t = t.replace("\\", "").replace("{", "").replace("}", "")
    return " ".join(t.split())   # 改行・タブ・連続する空白を1つの空白にまとめる
...
# ============ テロップの下敷き(プレート) ============
# 帯はASSの図形で描く。外部の画像素材は使わない
#  ・素材を足す運用だと、置き忘れ・ライセンス・解像度の3つで必ず詰まる
#  ・図形なら文字の長さに合わせて毎行ぴったりの幅になる(画像だと引き伸ばして角が歪む)
# 帯の上では強調色を黄色に変える。いまの濃緑(&H2A7A1E)は濃緑の帯に埋もれて読めない
TELOP_PLATE = "band"          # "band" 角丸帯 / "lower" 斜めカット帯 / "bubble" 吹き出し / "none" 帯なし
TELOP_PLATE_MAX = 1290        # 帯の最大幅。1430からキャラが立つので、そこに触らせない
_TELOP_CX  = (90 + (W - 640)) / 2.0    # \an2 の中心(MarginL 90 / MarginR 640 の中央)
_TELOP_CY  = (H - 84) - 112 / 2.0      # MarginV 84、1行ぶん112px の中心


def _ass_rrect(x, y, w, h, r):
    """角丸四角。ASSのbは制御点3つの3次ベジェ"""
    x2, y2 = x + w, y + h
    return (f"m {x+r:.0f} {y:.0f} l {x2-r:.0f} {y:.0f} "
            f"b {x2:.0f} {y:.0f} {x2:.0f} {y:.0f} {x2:.0f} {y+r:.0f} "
            f"l {x2:.0f} {y2-r:.0f} "
            f"b {x2:.0f} {y2:.0f} {x2:.0f} {y2:.0f} {x2-r:.0f} {y2:.0f} "
            f"l {x+r:.0f} {y2:.0f} "
            f"b {x:.0f} {y2:.0f} {x:.0f} {y2:.0f} {x:.0f} {y2-r:.0f} "
            f"l {x:.0f} {y+r:.0f} "
            f"b {x:.0f} {y:.0f} {x:.0f} {y:.0f} {x+r:.0f} {y:.0f}")


def _ass_shape(col, alpha, cmds):
    return (f"{{\\an7\\pos(0,0)\\bord0\\shad0\\c{col}&\\1a&H{alpha:02X}&\\p1}}"
            f"{cmds}{{\\p0}}")


# ショート(縦1080x1920)のテロップの置き場所。
# **YouTubeのショート再生画面は下から約20%(=384px)をUIが覆う**(タイトル・チャンネル名・シークバー。
# 2026-09-23に2つの記事で確認。右も約20%がボタン列、上は約10%)。
# 以前は帯を y1630〜1920 に置いていて、**テロップがまるごとUIの下に隠れていた**。
# 帯の下端を1470までに収め、CTAと挿絵もその上に積み直した
SHORT_PLATE = {"cx": 540, "cy": 1385, "maxw": 1000, "size": 72, "h": 170}   # cx は SW/2(SWの定義はもっと下)
# ショートに画像の下敷きを使うときの大きさ。縦画面は幅が狭いので横画面(1250x158)より小さい
SHORT_BASE_W, SHORT_BASE_H = 1000, 170
SHORT_CTA_Y  = 1120   # 登録の誘導の箱の上端(高さ約160なので1280で終わる)
SHORT_ART_BOT = 1090  # 挿絵の下端。CTAに触らせない
SHORT_BULLET_BOT = 1090   # 要点を描き終える下端


def telop_plate(text, scale, style=None, geom=None):
    """(帯のDialogue本文のリスト, 文字に足す上書きタグ, 強調色) を返す。
    style が "none" のときは帯なし = これまでのフチ2枚重ね。
    geom を渡すと縦画面(ショート)の座標で描く"""
    from PIL import ImageFont
    style = TELOP_PLATE if style is None else style
    if style == "none":
        return [], "", "&H002A7A1E"
    g = geom or {}
    cx   = g.get("cx", _TELOP_CX)
    cy   = g.get("cy", _TELOP_CY)
    maxw = g.get("maxw", TELOP_PLATE_MAX)
    size = g.get("size", TELOP_SIZE)
    tw = ImageFont.truetype(FONT_B, size).getlength(text) * ASS_RATIO * scale / 100.0
    out = []
    if style == "short":                       # 縦画面。角丸帯を文字幅に合わせる
        h, r, pad = g.get("h", 170), 24, 44
        w = min(tw + pad * 2, maxw)
        x, y = cx - w / 2, cy - h / 2
        out.append(_ass_shape("&H00080F04", 0x1E, _ass_rrect(x + 6, y + 9, w, h, r)))
        out.append(_ass_shape("&H00EFF5EA", 0x00, _ass_rrect(x, y, w, h, r)))
        out.append(_ass_shape("&H00152A0E", 0x08, _ass_rrect(x + 6, y + 6, w - 12, h - 12, r - 5)))
        out.append(_ass_shape("&H002A7A1E", 0x00, _ass_rrect(x + 6, y + 6, 18, h - 12, 7)))
        return out, r"{\c&HF6FAF2&\3c&H0A1606&\bord5\shad0}", "&H0066E8FF"
    if style == "lower":                       # 斜めカットの帯
        h, pad, cut = 122, 44, 26
        w = min(tw + pad * 2 + 74, maxw)
        x, y = cx - w / 2, cy - h / 2
        x2, y2 = x + w, y + h
        body = (f"m {x+62:.0f} {y:.0f} l {x2:.0f} {y:.0f} "
                f"l {x2-cut:.0f} {y2:.0f} l {x+62-cut:.0f} {y2:.0f}")
        shdw = (f"m {x+62:.0f} {y+7:.0f} l {x2:.0f} {y+7:.0f} "
                f"l {x2-cut:.0f} {y2+7:.0f} l {x+62-cut:.0f} {y2+7:.0f}")
        tab  = (f"m {x+cut:.0f} {y:.0f} l {x+56:.0f} {y:.0f} "
                f"l {x+56-cut:.0f} {y2:.0f} l {x:.0f} {y2:.0f}")
        out.append(_ass_shape("&H00101C0C", 0x30, shdw))
        out.append(_ass_shape("&H001B3A10", 0x0C, body))
        out.append(_ass_shape("&H002A7A1E", 0x00, tab))
        return out, r"{\c&HF6FAF2&\3c&H0A1606&\bord3\shad0}", "&H0066E8FF"
    if style == "bubble":                      # 吹き出し(右上にしっぽ)
        h, r, pad = 136, 34, 42
        w = min(tw + pad * 2, maxw)
        x, y = cx - w / 2, cy - h / 2
        tail = (f"m {x+w-150:.0f} {y+4:.0f} l {x+w-60:.0f} {y+4:.0f} "
                f"l {x+w-74:.0f} {y-44:.0f}")
        out.append(_ass_shape("&H00101C0C", 0x34, _ass_rrect(x + 6, y + 8, w, h, r)))
        out.append(_ass_shape("&H00FAFCF6", 0x00, _ass_rrect(x, y, w, h, r)))
        out.append(_ass_shape("&H00FAFCF6", 0x00, tail))
        out.append(_ass_shape("&H002A7A1E", 0x00, _ass_rrect(x + 22, y + h - 16, w - 44, 6, 3)))
        return out, r"{\c&H1B2C14&\3c&HFAFCF6&\bord2\shad0}", "&H002A7A1E"
    # 既定 = 角丸帯。濃い緑に白文字。長編の視聴者は平均55歳なので、面で塗るほうが読みやすい
    h, r, pad = 132, 20, 40
    w = min(tw + pad * 2, maxw)
    x, y = cx - w / 2, cy - h / 2
    out.append(_ass_shape("&H00101C0C", 0x2A, _ass_rrect(x + 5, y + 7, w, h, r)))
    out.append(_ass_shape("&H00F2F7EE", 0x00, _ass_rrect(x, y, w, h, r)))
    out.append(_ass_shape("&H001A2E12", 0x10, _ass_rrect(x + 5, y + 5, w - 10, h - 10, r - 4)))
    out.append(_ass_shape("&H002A7A1E", 0x00, _ass_rrect(x + 5, y + 5, 16, h - 10, 6)))
    return out, r"{\c&HF4F9F0&\3c&H0E1A08&\bord4\shad0}", "&H0066E8FF"


# ============ テロップの下敷きに画像を使う(2026-09-24。本人の「バリュエーション多いように」) ============
# イラストACのテロップベース(リポジトリの「イラストACテロップベース」フォルダ)から、
# 使えるデザインだけを切り出して回す。1枚に2〜10デザイン入ったシート形式なので箱で切り出す。
#  ・サンプル文字が焼き込まれた版と飾り素材は入れていない
#  ・**白文字か濃緑文字のどちらが読めるかは実測で決めた**。段数は「掛け算で暗く/スクリーンで明るく」
#    する強さで、1段=5%。**明暗差7.0以上**になるまで寄せてある(長編は平均55歳なので
#    アクセシビリティの最低基準4.5では足りないと判断した)
#  ・話者色もこの明暗で切り替える(明るい地=ずんだ濃緑/めたん濃紫、暗い地=ずんだ白/めたん淡桃)
TELOP_BASE_W, TELOP_BASE_H = 1250, 158
TELOP_BASE_ON = True            # False にすると図形で描く帯("band")に戻る
TELOP_BASES = [
    ("2124785.jpg",  (67, 29, 1522, 522),     True, 15),
    ("2124785.jpg",  (67, 624, 1522, 1117),   True, 15),
    ("2131776.jpg",  (87, 51, 1526, 374),     True,  8),
    ("2131776.jpg",  (87, 435, 1526, 758),   False,  9),
    ("2131776.jpg",  (87, 825, 1526, 1148),  False,  9),
    ("2137601.jpg",  (146, 194, 1460, 498),   True,  4),
    ("2137601.jpg",  (118, 672, 1491, 1037),  True,  4),
    ("27061135.jpg", (269, 591, 1871, 793),  False,  0),
    ("27061135.jpg", (2182, 591, 3784, 793), False,  2),
    ("27061135.jpg", (268, 924, 1871, 1126), False, 11),
    ("27061135.jpg", (2181, 924, 3784, 1126),False,  0),
    ("27061135.jpg", (269, 1258, 1871, 1460),False,  6),
    ("27061135.jpg", (2182, 1258, 3784, 1460),False, 4),
    ("27061135.jpg", (269, 1592, 1871, 1794),False,  8),
    ("27061135.jpg", (2182, 1592, 3784, 1794),False,10),
    ("27061135.jpg", (268, 1925, 1871, 2128),False, 10),
    ("27061135.jpg", (2182, 1925, 3783, 2128),False, 7),
    ("27495430.jpg", (167, 1528, 3734, 2156),False,  0),
    ("27595332.jpg", (167, 1528, 3734, 2156),False,  0),
]
_TB_CACHE = {}

# 明るい地のときの話者色(どちらも明暗差7.0を満たす濃さまで落としてある)
TB_INK   = {"zunda": "&H001B2C14", "guest": "&H003C1446"}
# 暗い地のときの話者色
TB_LIGHT = {"zunda": "&H00ECF8EE", "guest": "&H00F2DEFC"}


def _tb_nineslice(im, w, h, cap_ratio=0.10):
    """横方向だけ9スライスで伸ばす。両端のキャップを残して中央だけ引き伸ばすので飾りが歪まない"""
    from PIL import Image
    k = h / im.height
    im2 = im.resize((max(1, round(im.width * k)), h), Image.LANCZOS)
    cap = max(8, int(im2.width * cap_ratio))
    if w <= cap * 2 + 4:
        return im2.resize((w, h), Image.LANCZOS)
    L = im2.crop((0, 0, cap, h))
    R = im2.crop((im2.width - cap, 0, im2.width, h))
    M = im2.crop((cap, 0, im2.width - cap, h)).resize((w - cap * 2, h), Image.LANCZOS)
    o = Image.new("RGBA", (w, h), (0, 0, 0, 0))
    o.paste(L, (0, 0)); o.paste(M, (cap, 0)); o.paste(R, (w - cap, 0))
    return o


def telop_base(i, w=None, h=None):
    """i番目の下敷きを (画像, 白文字か) で返す。素材が無ければ (None, None)"""
    if not TELOP_BASE_ON or not TELOP_BASES:
        return None, None
    w = w or TELOP_BASE_W; h = h or TELOP_BASE_H
    key = (i % len(TELOP_BASES), w, h)
    if key in _TB_CACHE:
        return _TB_CACHE[key]
    fname, box, white, steps = TELOP_BASES[i % len(TELOP_BASES)]
    q = art_index().get(os.path.splitext(fname)[0])
    if not q:
        print(f"  ⚠ テロップの下敷きの素材が見つからないので図形で描きます: {fname}", flush=True)
        _TB_CACHE[key] = (None, None)
        return None, None
    try:
        from PIL import Image
        import numpy as np
        im = Image.open(q).convert("RGBA").crop(box)
        p = _tb_nineslice(im, w, h)
        if steps:
            f = max(0.05, 1.0 - steps * 0.05)
            a = np.asarray(p).astype(float)
            rgb = a[..., :3] / 255.0
            rgb = rgb * f if white else 1.0 - (1.0 - rgb) * f
            a[..., :3] = np.clip(rgb * 255.0, 0, 255)
            p = Image.fromarray(a.astype(np.uint8), "RGBA")
    except Exception as e:
        print(f"  ⚠ 下敷きを作れませんでした({fname}): {type(e).__name__}", flush=True)
        _TB_CACHE[key] = (None, None)
        return None, None
    _TB_CACHE[key] = (p, white)
    return p, white


def telop_base_tags(white):
    """(話者色の表, 強調色, 文字に足すフチの指定) を返す"""
    if white:
        return TB_LIGHT, "&H0066E8FF", r"\3c&H0A1606&\bord5\shad0"
    return TB_INK, "&H001A1AB0", r"\3c&HFAFCF6&\bord2\shad2\4c&H50000000&"


def short_base_no(script):
    """ショートは1本まるごとで1つの下敷き。回ごとに変える(中で変えるとちらつく)"""
    import zlib
    sp = script.get("short") or {}
    key = str(sp.get("title", "")) + "/" + str(script.get("chapter", ""))
    return zlib.crc32(key.encode()) % max(1, len(TELOP_BASES))


def telop_base_no(script, idx):
    """回ごとに始まりをずらして、シーンごとに1つずつ送る"""
    # hash() は実行ごとに変わるので使わない(同じ台本を作り直すと別の下敷きになってしまう)
    import zlib
    seed = zlib.crc32(str(script.get("chapter", "")).encode()) % max(1, len(TELOP_BASES))
    return seed + idx


def build_telops(entries, keywords, path, head=None, box=None, plate=None, geom=None):
    """entries: (start, end, text) / keywords: 強調する語のリスト
    plate: テロップの下敷き。長編は "band"(既定)、ショートは元から暗い帯があるので "none"

    層は下から 0=帯 / 1=フチと影 / 2=文字。帯があるときは1を省く(帯の上では溶ける)"""
    ...
    with open(path, "w", encoding="utf-8") as f:
        ...
        for e in entries:
            ...
            if len(e) > 5 and e[5]:
                # 画像の下敷きを使う回。帯はスライス側に焼き込んであるので図形は描かない。
                # 文字色は話者色(col)のまま、フチだけ地の明暗に合わせて差し替える
                shapes = []
                over = "{\\c" + col + "&" + e[5] + "}"
                kcol = e[6] if len(e) > 6 else "&H0066E8FF"
            else:
                shapes, over, kcol = telop_plate(body, sc, plate, geom)
            if over:
                col = None            # 帯があるときは文字色は over 側で決める
            for k in kws:
                if k and k in body:
                    body = body.replace(k, "\x01" + k + "\x02")
            if over:
                # 強調の終わりは \r でスタイルに戻してから、帯用の上書きを付け直す
                body = (body.replace("\x01", "{\\c" + kcol + "&\\b1}")
                            .replace("\x02", "{\\r}" + over + "{\\b1}"))
                body = over + body
            else:
                body = (body.replace("\x01", r"{\c&H2A7A1E&\b1}")
                            .replace("\x02", "{\\c" + (col or "&H001B2C14") + "&\\b1}"))
                if col and col != "&H001B2C14":
                    body = "{\\c" + col + "&}" + body
            anim = telop_anim(sc, e[4] if len(e) > 4 else "pop")
            plain = to_digits(esc(tx))
            for s in shapes:
                f.write(f"Dialogue: 0,{ass_time(st)},{ass_time(en)},Telop,,0,0,0,,"
                        f"{{\\fad(80,70)}}{s}\n")
            if not shapes and not (len(e) > 5 and e[5]):
                # 帯も画像の下敷きも無いときだけ、外側の暗いフチと影の層を敷く。
                # 画像の下敷きのときにこれを敷くと、文字に暗いにじみが出て濁る(2026-09-24に踏んだ)
                f.write(f"Dialogue: 1,{ass_time(st)},{ass_time(en)},TelopEdge,,0,0,0,,{anim}{plain}\n")
            f.write(f"Dialogue: 2,{ass_time(st)},{ass_time(en)},Telop,,0,0,0,,{anim}{body}\n")
...
# ============ 口調のチェックと直し ============
# ずんだもんの語り(narration)と who=="zunda" のセリフは「〜なのだ」「〜のだ」で終わる。
# 四国めたんは「〜なのよ」「〜かしら」「〜ね」など。台本を書くときに丁寧語(です・ます)が
# 混ざったり、めたんがずんだもん口調になることが実際にあったので、機械で直せるものは
# 直し、活用が必要で直せないものは警告を出して台本を書き直してもらう。
#
# 直すのはセリフと語りだけ。見出し・箇条書き・サムネの文字は画面に出す文なので触らない。

# 文の終わりだけを見る。文末の「よ」「ね」は先に落としてから判定する
_END = r"(?=[。、！？!?」』…]|$)"

# サ行五段動詞(「〜す」で終わる動詞)の語幹。「〜する」と区別するために列挙する
_SU = (r"(出|話|渡|貸|返|直|探|押|消|足|示|指|回|通|残|移|写|倒|試|逃|"
       r"増や|減ら|知ら|暮ら|過ご|生か|活か|果た|満た|飛ば|伸ば|延ば|崩|"
       r"表わ|現わ|費や|冷や|燃や)")

TONE_FIX = [
    # 文末の終助詞を先に落とす(「損しますよ」→「損します」)
    (r"ます(よ|ね|よね)" + _END,     "ます"),
    (r"ました(よ|ね|よね)" + _END,   "ました"),
    (r"ません(よ|ね|よね)" + _END,   "ません"),
    # い形容詞・「〜たい」「〜ない」の後は「なのだ」ではなく「のだ」。です系より先に置く
    (r"(?<=[^のな])いです(ね|よ|よね)?" + _END, "いのだ"),
    # 「〜のでしょうか」「〜んでしょうか」は の が重ならないように先に処理する
    (r"[のん]でしょうか" + _END,     "のだろうか"),
    (r"[のん]でしょうね?" + _END,    "はずなのだ"),
    (r"[のん]ですね?" + _END,        "のだ"),
    (r"[のん]ですよ" + _END,         "のだ"),
    # 丁寧語の否定・存在
    (r"ではありません" + _END,       "ではないのだ"),
    (r"じゃありません" + _END,       "じゃないのだ"),
    (r"ありません" + _END,           "ないのだ"),
    (r"ありました" + _END,           "あったのだ"),
    (r"あります" + _END,             "あるのだ"),
    (r"ございます" + _END,           "あるのだ"),
    ...
    (r"でしたね?" + _END,            "だったのだ"),
    (r"でした" + _END,               "だったのだ"),
    (r"です(ね|よ|よね)?" + _END,    "なのだ"),
    # 依頼・勧誘
    (r"([てで])ください" + _END,     r"\1ほしいのだ"),
    (r"ください" + _END,             "ほしいのだ"),
    # 「〜ましょう」もずんだもんは使わない。「ます」と同じ直し方で「のだ」にする
    (_SU + r"しましょう" + _END,     r"\1すのだ"),
    (r"しましょう" + _END,           "するのだ"),
    (r"([えけせてねめれげべぺで])ましょう" + _END, r"\1るのだ"),
    (r"(借|足|降|懲)りましょう" + _END, r"\1りるのだ"),
    (r"りましょう" + _END,           "るのだ"),
    (r"(起|生|尽|飽|過|で)きましょう" + _END, r"\1きるのだ"),
    (r"きましょう" + _END,           "くのだ"),
    (r"ぎましょう" + _END,           "ぐのだ"),
    # 「出します」は「出するのだ」ではなく「出すのだ」。サ行五段の動詞は
    # 「〜する」と同じ形に見えるので、よく使うものを先に並べて取り違えを防ぐ
    (_SU + r"しません" + _END,       r"\1さないのだ"),
    (_SU + r"します" + _END,         r"\1すのだ"),
    (r"しません" + _END,             "しないのだ"),
    (r"しました" + _END,             "したのだ"),
    (r"します" + _END,               "するのだ"),
    # ラ行・カ行の動詞。「上がります」→「上がるのだ」。一段動詞(借りる・起きる)だけは
    # 「りる」「きる」に戻すので、先に並べておく
    (r"(借|足|降|懲)ります" + _END,  r"\1りるのだ"),
    (r"ります" + _END,               "るのだ"),
    (r"(起|生|尽|飽|過|で)きます" + _END, r"\1きるのだ"),
    (r"きます" + _END,               "くのだ"),
    (r"過ぎます" + _END,             "過ぎるのだ"),
    (r"ぎます" + _END,               "ぐのだ"),
    (r"できません" + _END,           "できないのだ"),
    ...
    (r"なりました" + _END,           "なったのだ"),
    (r"なります" + _END,             "なるのだ"),
    # 一段動詞は「ます」を「る」に替えれば直せる。え段(え・け・せ・て・ね・め・れ・げ・べ・ぺ・で)で
    # 終わる語幹は五段動詞にはないので、これだけを機械で直す(「増えます」→「増えるのだ」、
    # 「増えています」→「増えているのだ」)。五段の「行きます」などは直せないので警告に回す
    (r"([えけせてねめれげべぺで])ません" + _END, r"\1ないのだ"),
    (r"([えけせてねめれげべぺで])ました" + _END, r"\1たのだ"),
    (r"([えけせてねめれげべぺで])ます" + _END,   r"\1るのだ"),
    # ずんだもんの語尾が壊れている形
    (r"会おうなのだ" + _END,         "会うのだ"),
    (r"どうぞなのだ" + _END,         "見てほしいのだ"),
    (r"(だ|よ)うなのだ" + _END,      r"\1のだ"),
]

def _to_metan(rep):
    """ずんだもん向けの置き換え先を、めたんの語尾に読み替える"""
    for a, b in (("のだろうか", "のかしら"), ("なのだ", "なのよ"), ("のだ", "のよ")):
        rep = rep.replace(a, b)
    return rep

# めたん用。中身は同じで語尾だけ違う。最後の2つで、めたんに混ざった
# ずんだもん口調(「〜なのだ」)を直す
METAN_FIX = [(pat, _to_metan(rep)) for pat, rep in TONE_FIX] + [
    (r"なのだ" + _END, "なのよ"),
    (r"のだ"   + _END, "のよ"),
]

# 語尾がこれで終わっていれば、その役の口調として正しい
_OK_ZUNDA = ("のだ", "なのだ", "のだろうか")
_OK_METAN = ("のよ", "なのよ", "かしら", "わね", "のね", "わよ", "ね", "よ", "の", "わ", "かな")
# セリフの合間に入る短いあいづち・感嘆。語尾が付かなくても不自然ではない
_INTERJ = ("あっ", "えっ", "ええっ", "うーん", "なるほど", "ふむ", "おお", "わあ",
           "たしかに", "確かに", "へえ", "ほう", "まあ", "うわっ", "そうなのだ", "そうなのよ")

def _sentences(text):
    return [x.strip() for x in re.split(r"(?<=[。！？!?])", str(text)) if x.strip()]

def fix_tone(text, who="zunda"):
    """セリフ・語りを担当キャラの口調に直す。
    戻り値は (直した文字列, 直しきれなかった文のリスト)"""
    zunda = (who == "zunda")
    t = str(text or "")
    for pat, rep in (TONE_FIX if zunda else METAN_FIX):
        t = re.sub(pat, rep, t)
    ok  = _OK_ZUNDA if zunda else _OK_METAN
    bad = []
    for sent in _sentences(t):
        core = sent.rstrip("。、！？!?」』… ")
        if not core or len(core) <= 5:
            continue
        if core.startswith(_INTERJ) or core.endswith(_INTERJ):
            continue
        if not core.endswith(ok):
            bad.append(sent)
    return t, bad
...
def fix_script(script):
    ...
    def fx(v):
        ...
        if w != v:
            hits.append((v, w))
        return w
    # 口調の直し。読み上げるものだけに使う(見出し・箇条書き・サムネの文字は画面に
    # 出す文なので、口調を付けるとおかしくなる)
    tone_bad = []
    def fz(v, who="zunda"):
        w, bad = fix_tone(fx(v), who)
        if w != v:
            hits.append((v, w))
        tone_bad.extend((who, b) for b in bad)
        return w
    for sc in script.get("scenes", []):
        for k in ("title", "kicker"):
            if sc.get(k):
                sc[k] = fx(sc[k])
        # 地の語りはずんだもんが読む
        if sc.get("narration"):
            sc["narration"] = fz(sc["narration"], "zunda")
        if sc.get("bullets"):
        ...
        for ln in sc.get("lines") or []:
            if isinstance(ln, dict) and ln.get("text"):
                ln["text"] = fz(ln["text"], ln.get("who") or "zunda")
            elif isinstance(ln, (list, tuple)) and len(ln) > 1 and isinstance(ln[1], str):
                ln[1] = fz(ln[1], ln[0] if len(ln) > 0 else "zunda")
    # フックとショートは fix_script を通っていなかったので、ここで一緒に直す
    meta = script.get("meta") or {}
    if meta.get("hook_say"):
        meta["hook_say"] = fz(meta["hook_say"], "zunda")
    if meta.get("hook_big"):
        meta["hook_big"] = [fx(x) if isinstance(x, str) else x for x in meta["hook_big"]]
    sh = script.get("short") or {}
    if isinstance(sh.get("title"), str) and sh.get("title"):
        sh["title"] = fx(sh["title"])
    if isinstance(sh.get("narration"), str) and sh.get("narration"):
        sh["narration"] = fz(sh["narration"], "zunda")
    if sh.get("bullets"):
        sh["bullets"] = [fx(b) if isinstance(b, str) else b for b in sh["bullets"]]
    for a, b in hits:
        print(f"  台本を修正: {a[:24]} -> {b[:24]}", flush=True)
    for who, sent in tone_bad:
        name = "ずんだもん" if who == "zunda" else "めたん"
        tail = "「〜なのだ」「〜のだ」" if who == "zunda" else "「〜なのよ」「〜かしら」「〜ね」"
        print(f"  ⚠ {name}の口調になっていないのだ: 「{sent[:34]}」"
              f" … {tail}で終わるように書き直してほしいのだ", flush=True)
...
def _finish_bed(out, marks, path, sr, normalize):
    ...
    # 尺が短いとフェードの長さが音より長くなって落ちていた。尺の1/3までに抑える
    fi = max(1, min(int(2.5 * sr), n // 3))
    fo = max(1, min(int(4.0 * sr), n // 3))
    if n:
...
def main(script_path, out_path, workdir=None):
    ...
    for i, (sc, items, scene_dur) in enumerate(plan, 1):
        # ★ テロップの下敷きはこのループで決める。
        # **main はシーンを2周する**(先に音声、あとでスライドとテロップ)。
        # 1周目で決めると、2周目には最後のシーンの値しか残っておらず、
        # 全シーンが同じ下敷きになる(2026-09-24に踏んだ)
        tb_img, tb_white = telop_base(telop_base_no(script, i))
        if tb_img is not None:
            tb_cols, tb_kcol, tb_edge = telop_base_tags(tb_white)
        else:
            tb_cols, tb_kcol, tb_edge = None, None, None
...
# 2026-09-10の実測(ショート34本、再生200回以上)で、尺と平均視聴時間の相関が +0.51 だった。
# 帯ごとの平均視聴: 〜24秒=4.0秒 / 25-27秒=4.1秒 / 28-30秒=6.0秒 / 31-34秒=6.7秒 / 35秒以上=7.3秒。
# 再生数も35秒以上の帯が最多(1225回)。短くするほど悪くなるので、狙いを38秒に上げた
# 2026-09-10の追加分析: 35秒以上の帯に上限は見えなかった。いちばん良かったのは
# 41秒(視聴9.7秒・再生1392・登録1人)と39秒(視聴10.3秒・再生1519)。
# ★ 2026-09-15の再分析で、上の「長いほど良い」は40秒で反転することが分かった。
#   同じ日に出した本どうしで比べると 40秒以上=平均720回 ↔ 28-36秒=825回 で、40秒超は不利。
#   36秒以上に絞ると955回 ↔ 777回で有利。つまり良いのは36〜40秒の帯だけ。
#   帯ごとの維持率も 32-36秒=20.2% / 36-40秒=17.8% / 40-44秒=15.2% で、40秒超が最下位。
#   1本平均再生も 32-36秒=932回 / 36-40秒=1050回 / 40-44秒=794回。
#   → 狙いを42秒から38秒に下げ、打ち切りも44秒にした(それ以上に伸ばす意味がない)
#   仕上がりの尺は narration の字数で決まる(実測5.2字/秒)。185〜195字・7文で36〜38秒になる。
#   打ち切りは「狙う尺」で起きるので、狙いは少し余裕を持たせた40秒にしてある
#   (38秒にすると195字の台本で最後の行動を促す1文が落ちる)
SHORT_MAX    = 44.0       # ここを超えたら打ち切る(2026-09-15に52.0から下げた)
SHORT_TARGET = 40.0       # 狙う尺(2026-09-15に42.0から下げた)
SHORT_GAP    = 0.18       # 文と文の間
SHORT_SPEED  = 1.00       # 速さは本編と同じ
SHORT_LOOP   = 0.85       # 末尾に冒頭の絵を戻してループを自然につなぐ秒数

# 登録の誘導。2026-09-10の実測で、ショートの平均視聴は5.6秒(尺30秒の18%)しかない。
# CTAを残り3.4秒からしか出していなかったため、**全再生の1.9%にしか届いていなかった**
# (届いた人の登録率は1.58%と高いので、誘導が弱いのではなく見られていなかった)。
# そこで「何が毎日届くのか」を上の帯に最初から出し、終わりぎわのCTAも早めて2行にした。
# 上の帯にしたのは、ショートの再生画面はYouTube側のUIが下と右を覆うため
SHORT_PROMISE  = "毎日 お金の注意報"   # 常に出す約束。アイコン・バナー・概要欄と同じ文言にする
SHORT_BADGE_IN = 0.0      # 何秒目からバッジを出すか。平均視聴が5.6秒なので0にしてある
SHORT_CTA_IN   = 6.0      # 終わりぎわのCTAを出しはじめる残り秒数
...
[V4+ Styles]
Format: Name,Fontname,Fontsize,PrimaryColour,SecondaryColour,OutlineColour,BackColour,Bold,Italic,Underline,StrikeOut,ScaleX,ScaleY,Spacing,Angle,BorderStyle,Outline,Shadow,Alignment,MarginL,MarginR,MarginV,Encoding
; ショートも角丸帯の上の白文字にした(2026-09-23i)。MarginV 485 で帯の中心が y1385 に来る。
; 以前は118で、テロップが**YouTubeのUIに隠れる下から20%の中**にあった
Style: TelopEdge,Noto Sans CJK JP,72,&H00101808,&H000000FF,&H00101808,&H78000000,-1,0,0,0,100,100,1,0,1,10,6,2,50,50,485,1
Style: Telop,Noto Sans CJK JP,72,&H00FFFFFF,&H000000FF,&H00203010,&H00FFFFFF,-1,0,0,0,100,100,1,0,1,10,0,2,50,50,485,1
...
def short_spec(script):
    ...
    if not sp.get("title"):
        # 1コマ目に「結論」の札つきで出る見出し。空だと札だけが出て間が抜けるので、
        # フックの大見出し → 長編のタイトル → シーン1の見出し の順に拾う
        for cand in ((meta.get("hook_big") or [None])[0],
                     (meta.get("titles") or [None])[0],
                     (scenes[0].get("title") if scenes else None)):
    ...
    sp["bullets"] = [b for b in (sp.get("bullets") or []) if isinstance(b, str)][:3]
    if not sp.get("art"):
        # 挿絵。short.art が無ければ本編のシーンから最初の1枚を借りる
        # (ショートの平均視聴は5.6秒なので、文字だけの画面より絵が1枚あるほうが止まる)
        for sc in scenes:
            if sc.get("art"):
                sp["art"] = sc["art"]
                break
...
def cmd_inputs(cmd):
    """ffmpeg のコマンドに並んでいる -i の数(次の入力の番号になる)"""
    return [a for a in cmd if a == "-i"]


def draw_short_slide(sp, chapter, path, shown, progress, acc, remain=None, elapsed=None,
                     telop_base_img=None):
    """縦画面の1枚。上に見出し、真ん中に要点、下は立ち絵とテロップ。
       remain は残り秒数、elapsed は経過秒数。残り時間バーは出さない(残りが見えると離脱するため)"""
    ...
    d.text((44, 40), chapter[:12], font=ImageFont.truetype(FONT_R, 34), fill=INK_SOFT)

    # 何が毎日届くのかを上の帯に常に出す。ショートの平均視聴は5.6秒しかないので、
    # ここに出さないと登録の話が誰にも届かない(実測で終わりぎわのCTAは1.9%にしか届いていなかった)
    if elapsed is None or elapsed >= SHORT_BADGE_IN:
        pa = 255      # コマは静止画なのでコマ内でフェードはできない。常に出し切る
        f_pr = ImageFont.truetype(FONT_B, 32)
        pw = int(d.textlength(SHORT_PROMISE, font=f_pr))
        px2, pyb = SW - 44, 26
        d.rounded_rectangle([px2 - pw - 76, pyb, px2, pyb + 62], radius=31, fill=acc + (pa,))
        # 登録を連想させる小さな丸(YouTubeのボタンは下にあるので、ここは目印だけ)
        d.ellipse([px2 - pw - 66, pyb + 15, px2 - pw - 34, pyb + 47],
                  fill=(255, 255, 255, pa))
        d.text((px2 - pw - 50, pyb + 31), "＋", font=ImageFont.truetype(FONT_B, 26),
               fill=acc + (pa,), anchor="mm")
        d.text((px2 - pw - 20, pyb + 31), SHORT_PROMISE, font=f_pr,
               fill=(255, 255, 255, pa), anchor="lm")
    ...
    for sz in (52, 46, 42, 38):
        ...
        if sum(len(r) for r in rows) * int(sz * 1.34) + len(rows) * 30 <= SHORT_BULLET_BOT - y:
            break
    # ★ 挿絵の位置は「要点を全部出したときの下端」から測る。
    # 表示ぶん(n)だけで測ると、要点が1つずつ増えるコマごとに絵の大きさと位置が変わって
    # ぱたぱた動いて見える(要点は読み上げに合わせて増えるので n はコマごとに違う)
    y_all = y + sum(len(r) for r in rows) * int(sz * 1.34) + len(rows) * 30
    ...
    # 挿絵(いらすとや)。**立ち絵は x=590 から立つので左の空きに置く**。
    # 下は登録の誘導の箱(y=SH-660)に触らせない。CTAは終わりぎわだけ出るが、
    # 絵の位置がコマごとに動くとちらつくので、**CTAが無いコマでも同じ場所に置く**
    art_name = sp.get("art")
    if art_name:
        a_top, a_bot = y_all + 24, SHORT_ART_BOT   # y_all は要点を全部出したときの下端
        a_h = min(ART_MAX, a_bot - a_top)
        # 260pxを下回ると絵が小さすぎて何の絵か分からない(実測。長編の図解と同じ下限)
        if a_h < 260:
            print(f"  ⚠ ショートの挿絵を置く高さが{max(0, a_bot - a_top)}pxしかないので省いた: "
                  f"{art_name}(見出しか要点を短くする)")
        else:
            art_im = load_art(art_name, a_h)
            if art_im is None:
                print(f"  ⚠ ショートの挿絵が見つからないのだ: {art_name}")
            else:
                if art_im.width > 515:            # 立ち絵(x=590から)に触れない幅に収める
                    art_im = art_im.resize(
                        (515, max(1, round(art_im.height * 515 / art_im.width))),
                        Image.LANCZOS)
                # 左端は見出しと同じ55に合わせる(40だと文字より外に出て落ち着かない)
                ax = 55 + (515 - art_im.width) // 2
                ay = a_top + (a_bot - a_top - art_im.height) // 2
                img.alpha_composite(art_im.convert("RGBA"), (ax, ay))

    # 終わりぎわの登録の誘導。SHORT_CTA_IN 秒前から出す(3.4秒だと到達が1.9%しかなかった)
    if remain is not None and remain <= SHORT_CTA_IN:
        a = int(235 * min(1.0, (SHORT_CTA_IN - remain) / 0.8))
        # 立ち絵は右下(x=590〜)に重なるので、左側の空きに置く
        t1, t2 = "登録するのだ", SHORT_PROMISE
        for cs in (52, 48, 44, 40, 36, 32):
            f_c1 = ImageFont.truetype(FONT_B, cs)
            f_c2 = ImageFont.truetype(FONT_R, int(cs * 0.72))
            tw = max(d.textlength(t1, font=f_c1), d.textlength(t2, font=f_c2))
            if tw <= 470:
                break
        bw = int(tw) + 68
        bh = int(cs * 1.30) + int(cs * 0.72 * 1.30) + 34
        bx, by = 40, SHORT_CTA_Y
        d.rounded_rectangle([bx, by, bx + bw, by + bh], radius=30, fill=acc + (a,))
        cy1 = by + 17 + int(cs * 1.30) // 2
        d.text((bx + bw // 2, cy1), t1, font=f_c1, fill=(255, 255, 255, a), anchor="mm")
        d.text((bx + bw // 2, cy1 + int(cs * 1.30) // 2 + int(cs * 0.72 * 1.30) // 2 + 4),
               t2, font=f_c2, fill=(255, 255, 255, min(a, 225)), anchor="mm")

    # ★ 下の全幅の帯はやめた(2026-09-23i)。y1630〜1920 は YouTube の UI が覆う帯で、
    # そこに敷いても誰にも見えていなかった。テロップは `SHORT_PLATE` の角丸帯が背負う
    # 残り時間バーは出さない。あと何秒あるかが見えると、そこで離脱されるため

    # 画像の下敷き(2026-09-24b)。ASSの角丸帯の代わりに、同じ場所へ焼き込む
    if telop_base_img is not None:
        bw, bh = telop_base_img.size
        img.alpha_composite(telop_base_img.convert("RGBA"),
                            (int(SHORT_PLATE["cx"]) - bw // 2,
                             int(SHORT_PLATE["cy"]) - bh // 2))
...
def build_short(script_path, out_path, workdir=None):
    ...
    if len(items) < len(sents):
        print(f"  ナレーションが長いので{len(items)}/{len(sents)}文で打ち切り "
              ...
              f"7文・185〜195字に収めるとよい(実測5.2字/秒で36〜38秒)")

    telops, mouth_seq, slide_seq, audio, t = [], [], [], 0.0, 0.0
    nb = len(sp.get("bullets", [])[:4])
    # テロップの下敷き。ショートは1本まるごとで1つ(中で変えるとちらつく)
    sb_img, sb_white = telop_base(short_base_no(script), SHORT_BASE_W, SHORT_BASE_H)
    if sb_img is not None:
        sb_cols, sb_kcol, sb_edge = telop_base_tags(sb_white)
        sb_col = sb_cols["zunda"]
    else:
        sb_col, sb_kcol, sb_edge = "&H00FFFFFF", None, None
    for j, (e2, sent, wav, q, dur) in enumerate(items):
        ...
        for st, en, tx in align_telops(q, sent, 16, dur):
            telops.append((t + st, t + en, tx, sb_col, fx, sb_edge, sb_kcol))
        png = f"{workdir}/f{j:02d}.png"
        shown = nb if len(items) <= 1 else min(nb, j)
        draw_short_slide(sp, chapter, png, shown, (t + dur) / clock, acc,
                         remain=clock - (t + dur), elapsed=t)
        slide_seq.append((png, dur + GAP))
        t += dur + GAP
    ...
    with open(f"{workdir}/c.txt","w") as f:
        ...
        f.write(f"file '{mouth_seq[-1][0]}'\n")
    # 画像の下敷きがあるときは図形の帯を描かない(スライドに焼き込んであるため)
    build_telops(telops, [], f"{workdir}/telop.ass", head=SHORT_ASS, box=980,
                 plate=("none" if sb_img is not None else "short"), geom=SHORT_PLATE)
    ...
    if bed:
        cmd += ["-i", bed]
    # ★ テロップの下敷きは**立ち絵より後ろに重ねる**。スライドPNGに焼き込むと、
    # 立ち絵(x=590〜1060)が帯の上に来て帯が切れて見える(2026-09-24に踏んだ)
    base_in = None
    if sb_img is not None:
        base_png = f"{workdir}/tbase.png"
        sb_img.convert("RGBA").save(base_png)
        base_in = len(cmd_inputs(cmd))
        cmd += ["-loop", "1", "-i", base_png]
    vf = (f"[0:v]scale={SW}:{SH},fps=30[bg];"
          f"[1:v]scale={CW2}:{CH2},fps=30[ch];"
          f"[bg][ch]overlay={SW-CW2-20}:{SH-CH2-210}:shortest=0"
          + ("[v0];" if base_in is not None else "[v];"))
    if base_in is not None:
        bx = int(SHORT_PLATE["cx"]) - sb_img.width // 2
        by = int(SHORT_PLATE["cy"]) - sb_img.height // 2
        vf += f"[v0][{base_in}:v]overlay={bx}:{by}:shortest=1[v];"
    vf += f"[v]ass={workdir}/telop.ass,format=yuv420p[vo]"
    if bed:
