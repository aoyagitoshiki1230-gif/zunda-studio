...
def register_readings():
    ...
    print(f"  読み辞書 {n}語を登録({'/'.join(su for su, _, _ in READINGS)})")
    return n
...
def kana_moras(yomi):
    """カタカナの読みからモーラ数を数える(小さいャュョは前の字と1モーラ)"""
    return sum(1 for c in yomi if c not in SMALL_KANA)
...
def _apply_readings(text, spans):
    ...
    for su, yomi, _ in READINGS:
        ...
        if not key:
            continue
        want, i = kana_moras(yomi), low.find(key)
        while i >= 0:
            j = i + len(key)
