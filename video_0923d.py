...
def build_telops(entries, keywords, path, head=None, box=None):
    ...
    with open(path, "w", encoding="utf-8") as f:
        ...
        for e in entries:
            ...
            # 下の層は外側の暗いフチと影だけを描かせる。文字色の指定は上の層が覆うので外す
            plain = to_digits(esc(tx))
            f.write(f"Dialogue: 0,{ass_time(st)},{ass_time(en)},TelopEdge,,0,0,0,,{anim}{plain}\n")
            f.write(f"Dialogue: 1,{ass_time(st)},{ass_time(en)},Telop,,0,0,0,,{anim}{body}\n")
