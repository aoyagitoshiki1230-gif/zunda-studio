...
def telop_plate(text, scale, style=None):
    ...
    if style == "none":
        return [], "", "&H002A7A1E"
    tw = ImageFont.truetype(FONT_B, TELOP_SIZE).getlength(text) * ASS_RATIO * scale / 100.0
    out = []
    if style == "lower":                       # 斜めカットの帯
        h, pad, cut = 122, 44, 26
        w = min(tw + pad * 2 + 74, TELOP_PLATE_MAX)
        x, y = _TELOP_CX - w / 2, _TELOP_CY - h / 2
        x2, y2 = x + w, y + h
    ...
    if style == "bubble":                      # 吹き出し(右上にしっぽ)
        h, r, pad = 136, 34, 42
        w = min(tw + pad * 2, TELOP_PLATE_MAX)
        x, y = _TELOP_CX - w / 2, _TELOP_CY - h / 2
...
def build_telops(entries, keywords, path, head=None, box=None, plate=None):
    ...
    with open(path, "w", encoding="utf-8") as f:
        ...
        for e in entries:
            ...
            shapes, over, kcol = telop_plate(body, sc, plate)
            if over:
...
[V4+ Styles]
Format: Name,Fontname,Fontsize,PrimaryColour,SecondaryColour,OutlineColour,BackColour,Bold,Italic,Underline,StrikeOut,ScaleX,ScaleY,Spacing,Angle,BorderStyle,Outline,Shadow,Alignment,MarginL,MarginR,MarginV,Encoding
; ショートは暗い帯の上に白文字なので、外フチは背景に溶ける。下の層は影だけを足す役(フチは上と同じ10)
Style: TelopEdge,Noto Sans CJK JP,72,&H00101808,&H000000FF,&H00101808,&H78000000,-1,0,0,0,100,100,1,0,1,10,6,2,50,50,118,1
...
def draw_short_slide(sp, chapter, path, shown, progress, acc, remain=None, elapsed=None):
    ...
    art_name = sp.get("art")
    if art_name:
        a_top, a_bot = y_all + 24, SH - 660 - 24   # y_all は要点を全部出したときの下端
