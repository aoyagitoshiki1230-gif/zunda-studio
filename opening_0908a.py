...
def _fit_font(path, text, maxw, hi, lo=34):
    """maxw に収まる大きさを選ぶ。下限で止めるとはみ出すので、下限を割ってでも収める"""
    from PIL import ImageFont
    sz = hi
    while sz > lo and ImageFont.truetype(path, sz).getlength(str(text)) > maxw:
        sz -= 4
    while sz > 12 and ImageFont.truetype(path, sz).getlength(str(text)) > maxw:
        sz -= 2
    if sz < lo:
        print(f"  OPの文字が長すぎるので{sz}pxまで縮めました: {str(text)[:18]}")
    return sz
...
def _ensure_readings():
    """iDeCo・お得などの読みをVOICEVOXに入れておく(video.py の READINGS)"""
    if _DICT_DONE["v"]:
        return
    _DICT_DONE["v"] = True
    try:
        ...
        print(f"  読み辞書を用意できなかったので既定の読みで続行: {type(e).__name__}")
...
def build(out_path, channel_name="ずんだもん解説", tagline="気になるあれこれを10分で",
    ...
    # OPが長いほど本編に入るのが遅れる。実測で本編の平均視聴は57秒しかなく、
    # フック+OPで22秒使っていたので、既定のOPナレーションはチャンネル名だけにした
    narration = narration or f"{channel_name}なのだ。"

    wav = os.path.join(workdir, "op.wav")
    q, vdur = _synth(voicevox, narration, speaker, wav)
    dur = max(3.6, vdur + 0.8)
    n = int(dur * FPS)
    ...
    MAXW = 1120
    base_sz = 150
    base_sz = _fit_font(FB, channel_name, MAXW, base_sz, 60)
    while False:
        base_sz -= 4
    f_name = ImageFont.truetype(FB, base_sz)
    f_tag  = ImageFont.truetype(FM, _fit_font(FM, tagline, MAXW, 54, 34))
...
def build_ending(out_path, channel_name="ずんだもん生活の知恵",
    ...
    MAXW = 1040
    sz_name = 96
    sz_name = _fit_font(FB, channel_name, MAXW, sz_name, 44)
    while False:
        sz_name -= 3
...
def build_hook(out_path, say, big_lines, speaker=3,
    ...
    for t in big_lines:
        sizes.append(_fit_font(FB, t, MAXW, top, 52))
