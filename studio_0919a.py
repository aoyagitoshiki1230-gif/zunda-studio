...
出力:
    out/video.mp4
    out/thumbnail_alert.jpg / _guide.jpg / _number.jpg / _duo.jpg
...
def main():
    ...
    print("\n[動画] 生成開始")
    import video as V
    # video.main は台本を自分で読み直して fix_script をかける。ここの script は
    # 別物なので、同じ直しをかけておかないとフック(hook_say/hook_big)とサムネだけ
    # 元の文字のままになる(「20〜50代」の波ダッシュが読み飛ばされるなど)
    V.fix_script(script)
    ...
    if not thumbs:
        print("  ⚠ サムネイルが1枚も作れなかったのだ。"
              "台本の thumbnail に alert / number / guide / duo のどれかを入れてほしいのだ")
